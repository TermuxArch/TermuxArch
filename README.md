...
This BASH setup shell script will attempt to set Arch Linux up in a Termux PRoot environment with or without QEMU support.  Emulated computer architectures `armeabi-v7a`, `x86` and `x86-64` are supported on `arm64-v8a` architecture smartphones.  Emulated computer architecture `x86` is supported on `armeabi-v7a` architecture smartphones.  The command `setupTermuxArch h[elp]` has more information.
