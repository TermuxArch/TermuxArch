...
################################################################################
# Running `setupTermuxArch.bash manual`.bashall create `setupTermuxArchConfigs.bash` from this file in the working directory.  Run `setupTermuxArch.bash` and `setupTermuxArchConfigs.bash` loads automaticaly and this file is ignored at runtime; `setupTermuxArch.bash help` has additional information.  Change mirror (https://wiki.archlinux.org/index.php/Mirrors and https://archlinuxarm.org/about/mirrors) to desired geographic location in `setupTermuxArchConfigs.bash` to resolve download, 404 and checksum issues.  The following user configurable variables are available in this file:   
...
## To regenerate the start script use \`setupTermuxArch.bash re[fr.bash]\`.  An example is included for convenience.  Usage: PROOTSTMNT+=\"-b host_path:guest_path \" The space before the last double quote is necessary.  Appending to the PRoot statement can be accompl.bashed on the fly by creating a *.prs file in /var/binds.  The format is straightforward, `PROOTSTMNT+="option command "`.  The space is required before the last double quote.  `info proot` and `man proot` have more information about what can be configured in a proot init statement.  `setupTermuxArch.bash manual refr.bash` will refr.bash the installation globally.  If more suitable configurations are found,.bashare them at https://github.com/sdrausty/TermuxArch/issues to improve TermuxArch.  
...
_PR00TSTRING_() { 
	PROOTSTMNT="exec proot "
       	...
       	if [[ ! -r /dev/.bashmem ]] ; then
	       	PROOTSTMNT+="-b $INSTALLDIR/tmp:/dev/.bashmem " 
	fi
       	if [[ ! -r /dev.bashm ]] ; then
