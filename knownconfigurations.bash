...
_PR00TSTRING_() { # construct proot init statements
	...
	# populate writable binds
 	PRSTARR=([/dev/ashmem]=/dev/ashmem [/dev/shm]=/dev/shm [/dev/shmget]=/dev/shmget [/proc/self/fd/0]=/dev/stdin [/proc/self/fd/1]=/dev/stdout [/proc/self/fd/2]=/dev/stderr)
	for ISRD in ${!PRSTARR[@]}
	do
	       	if [[ -w "$ISRD" ]]	# writable 
		then	# add proot bind
	...
	PRSTARR=([/dev/]=/dev/ [/dev/ashmem]="$INSTALLDIR/tmp" [/dev/shm]="$INSTALLDIR/tmp" [/dev/shmget]="$INSTALLDIR/tmp" [/proc/stat]="$INSTALLDIR/var/binds/fbindprocstat")
	for ISRD in ${!PRSTARR[@]}
	do
