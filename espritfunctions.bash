...
addlangq() {
	while true; do
		printf "\\e[1;34m  Add languages to the Arch Linux system? To edit \\e[1;32m/etc/locale.gen\\e[1;34m for your preferred language(s) before running \\e[1;32mlocale-gen\\e[1;34m choose edit.  Would you like to run \\e[1;32mlocale-gen\\e[1;34m with the Engl.bash en_US.UTF-8 locale only?  "
		read -n 1 -p "Answer yes to generate the Engl.bash en_US.UTF-8 locale only [Y|e] " ye
		if [[ "$ye" = [Yy]* ]] || [[ "$ye" = "" ]];then
			break
...
rmbloomq() {
	if [[ -d "$HOME"/TermuxArchBloom ]];then
		...
		while true; do
			printf "\\n\\e[1;30m"
			read -n 1 -p "Refr.bash $HOME/TermuxArchBloom? [Y|n] " rbuanswer
...
_RUNFINISHSETUPQ_() {
	while true; do
		printf "\\n\\e[0;32mWould you like to run \\e[1;32mfin.bashsetup.bash\\e[0;32m to complete the Arch Linux configuration and update now, or at a later time?  \\e[1;32mNow is recommended.  \\e[0;32m"
		read -n 1 -p "Answer yes to complete the Arch Linux configuration and update now; Or answer no for later [Y|n] " nl
	if [[ "$nl" = [Yy]* ]] || [[ "$nl" = "" ]];then
		_RUNFINISHSETUP_ 
		break
	elif [[ "$nl" = [Nn]* ]];then
		printf "\\n\\e[0;32mSet the geographically nearby CMIRROR in \\e[1;32m/etc/pacman.d/mirrorlist\\e[0;32m first.  Then use \\e[1;32m$INSTALLDIR/root/bin/setupbin.bash\\e[0;32m in Termux to run \\e[1;32mfin.bashsetup.bash\\e[0;32m or simply \\e[1;32mfin.bashsetup.bash\\e[0;32m in Arch Linux Termux PRoot to complete the Arch Linux configuration and update."
		break
	else
