...
_BKPPROOT_() {
    ...
    if [ ! -f "$HOME/termux/proot/tmp/proot.bkp" ] && [ -f "$PREFIX/bin/proot" ]; then
...
_GITCLONETERMUXPROOT_() {
    if [[ -z "${GITCLONETERMUXPROOTLCR:-}" ]]; then
...
_MAKEV1_() {
    if [ -f "$HOME/termux/proot/src/builttaprootuserland.lock" ]; then
        ...
        if ! grep DUSERLAND GNUmakefile; then
...
_DOTAPROOTUSERLAND_() {
    if [ ! -e "$HOME/termux/proot/src" ]; then
...
if [[ -z "${1:-}" ]]; then
    _DOTAPROOTUSERLAND_
elif [[ "${1//-/}" = [Rr][Ee][Vv]* ]]; then ##  [rev[ert] build from commit:
    GITCLONETERMUXPROOTLCR="bc3668f3238cee2011f9afa5a964b2dfc9dc523b"
    _DOTAPROOTUSERLAND_
elif [[ "${1//-/}" = [Rr][Ee]* ]]; then ##  [re[vert] build from commit:
    GITCLONETERMUXPROOTLCR="292fbc8a5406fdaf17b530444cd3dbaa92b1551d"
    _DOTAPROOTUSERLAND_
elif [[ "${1//-/}" = [Rr]* ]]; then ##  [r[evert] from backup file:
    if [ -f "$HOME/termux/proot/tmp/proot.bkp" ]; then
