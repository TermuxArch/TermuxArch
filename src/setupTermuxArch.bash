...
_CHK_() {
    if sha512sum -c --quiet termuxarchchecksum.sha512; then
        if [[ -z "${INSTALLDIR:-}" ]]; then # is unset
            # exit here or the program will run on
...
_CHKSELF_() {
    # compare setupTermuxArch and file being used
    cd "$WFDIR"                                                           # change directory to working file directory
    if [[ "$(< "$TAMPDIR"/setupTermuxArch)" != "$(< "${0##*/}")" ]]; then # differ
        # update the working file to newest version
        ## update working file
        cd "$WDIR"
...
_DEPENDDM_() {
    # check and set download manager
    for PKG in "${!ADM[@]}"; do
        if [[ -x $(command -v "${ADM[$PKG]}") ]]; then
            DM="$PKG"
...
_DEPENDIFDM_() {
    # check if download tool is available and set for install
    for PKG in "${!ADM[@]}"; do # check from available toolset and set one for install
        #	check for both set DM and if tool exists on device
        if [[ "$DM" = "$PKG" ]] && [[ ! -x $(command -v "${ADM[$PKG]}") ]]; then #	sets both download tool for install and exception check.
            PKGS+=($PKG)
...
_DEPENDS_() {
    ...
    _DEPENDS_() {
        ...
        if [[ "$COMMANDIF" = au ]]; then # can enable rollback https://wae.github.io/au/
            # use 'au' to install missing packages
            au "${PKGS[@]}" && printf "$STRNGB%s" "$STRING1F" || printf "$STRNGC%s" "$STRING2"
        elif [[ "$COMMANDIF" = pkg ]]; then # use 'pkg' to install missing packages
            pkg install "${PKGS[@]}" printf '%s' "$STRNGC $STRING1" || printf '%s' "$STRNGC $STRING2"
        elif [[ "$COMMANDIF" = apt ]]; then # use 'apt' to install missing packages
...
_DWNL_() {
    # download TermuxArch from Github
    ...
    if [[ "$DM" = aria2 ]]; then # use https://github.com/aria2/aria2
        "${ADM[aria2]}" -Z "${FILE[sha]}" "${FILE[tar]}"
    elif [[ "$DM" = axel ]]; then # use https://github.com/mopp/Axel
        "${ADM[axel]}" -a "${FILE[sha]}"
        "${ADM[axel]}" -a "${FILE[tar]}"
    elif [[ "$DM" = curl ]]; then # use https://github.com/curl/curl
        "${ADM[curl]}" "$DMVERBOSE" -O {"${FILE[sha]},${FILE[tar]}"}
    elif [[ "$DM" = wget ]]; then # use https://github.com/mirror/wget
        _DOADMWGET_() {
...
_EDITORCHOOSER_() {
    # add 'export EDITOR=editor_name' to HOME/.bash_profile in order to use your favorite editor during runtime
    if [[ -z "${EDITOR:-}" ]]; then
...
_INTROBLOOM_() {
    # BLOOM = setupTermuxArch manual verbose
    OPT=BLOOM
...
_DODIRCHK_() {
    _SETROOT_EXCEPTION_
    if [ ! -d "$INSTALLDIR" ] || [ ! -d "$INSTALLDIR/root/bin" ] || [ ! -d "$INSTALLDIR/var/binds" ] || [ ! -f "$INSTALLDIR/bin/we" ] || [ ! -f "$INSTALLDIR/usr/bin/env" ]; then
        ...
        if [ -d "$INSTALLDIR"/tmp ]; then # check for superfluous tmp directory
            DIRCHECK=0
            DIRNAME=(dev etc home opt proc root sys usr var)
            for IDIRNAME in "${DIRNAME[@]}"; do
                if [ ! -d "$INSTALLDIR/$IDIRNAME" ]; then
                    DIRCHECK=1
                else
                    DIRCHECK=0
                fi
        ...
        elif [ "$DIRCHECK" -eq 1 ]; then # delete superfluous tmp dir
...
_INSTLLDIRCHK_() {
    if [[ -f "$INSTALLDIR"/bin/we ]] && [[ -d "$INSTALLDIR"/usr/local/termuxarch/bin ]] && [[ -d "$INSTALLDIR"/root/bin ]] && [[ -d "$INSTALLDIR"/var/binds ]]; then
...
_QEMU_() {
    ...
    select ARCHITECTURE in exit armv7 arm64-v8a i386 x86_64; do
...
_RMARCHQ_() {
    ...
    while true; do
        ...
        if [[ "$RUANSWER" = [Ee]* ]] || [[ "$RUANSWER" = [Nn]* ]] || [[ "$RUANSWER" = [Qq]* ]]; then
            ...
            if grep -q ^pacmd "$PREFIX/etc/profile" && grep -q ^pulseaudio "$PREFIX/etc/profile"; then
...
_TAMATRIXEXIT_() {
    # run when Matrix presentation ends
    if [[ -n "${TAMATRIXENDLCR:-}" ]]; then
        _TAMATRIXEND_
    fi
}
...
_IFBINEXT_() {
    if [ -d "$HOME/bin" ] && grep "$HOME/bin" <<< "$PATH"; then
...
## [q[emu] [m[anual]] [i[nstall]|r[e[f[resh]]]] [customdir]]  Install alternate architecture on smartphone with https://github.com/qemu/QEMU emulation.  Issue [Implementing QEMU #25](https://github.com/TermuxArch/TermuxArch/issues/25) has more information.
    ...
    _QEMU_
    _INTRO_ "$@"
elif [[ "${1//-/}" = [Rr][Ee][Ff][Rr][Ee]*[Ff][Oo][Rr][Cc][Ee] ]]; then
...
elif [[ "${1//-/}" = [Rr][Ee][Ff][Rr][Ee]* ]]; then
    printf '\nSetting mode to full refresh.\n'
    _PRPREFRESH_ "5"
    _ARG2DIR_ "$@"
    _INTROREFRESH_ "$@"
elif [[ "${1//-/}" = [Rr][Ee][Ff][Rr]* ]]; then
    printf '\nSetting mode to 4 refresh.\n'
    _PRPREFRESH_ "4"
    _ARG2DIR_ "$@"
    _INTROREFRESH_ "$@"
