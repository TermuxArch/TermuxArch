...
_DOMIRROR_() {
    # partial implementaion: choose the corrrect mirror and test this mirror website
    _DOCEMIRROR_() {
        COUNTRYCODEISO="$(getprop gsm.operator.iso-country)"
        USERCOUNTRYCODE="${COUNTRYCODEISO%%[\ \,]*}"
        if [[ -z "${USERCOUNTRYCODE:-}" ]]; then
    ...
    if [[ -f "$INSTALLDIR/run/lock/${INSTALLDIR##*/}/domirror.lock" ]]; then
        printf "Lockfile '%s' exists;  Continuing..." "$INSTALLDIR/run/lock/${INSTALLDIR##*/}/domirror.lock"
    else
        if ! grep ^Server "$INSTALLDIR/etc/pacman.d/mirrorlist"; then
            _DOCEMIRROR_
        fi
    fi
}
...
_MD5CHECK_() {
    if md5sum -c --quiet "$IFILE".md5 2> /dev/null; then
        ...
        if md5sum -c --quiet ./*.md5 2> /dev/null; then
            ...
            {
                [[ "$KEEP" = 0 ]] && _PRINTKEEPEXIT_
                exit 203
            } || {
                _PRINTMD5ERROR_ && rm -f "$INSTALLDIR"/*.tar.gz "$INSTALLDIR"/*.tar.gz.md5
                exit 205
            }
        fi
    fi
