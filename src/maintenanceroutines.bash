...
_DOFUNLCR2_() {
    ...
    _BKPTHF_() {
        # backup file to backup file directory
    ...
    _CPYFRT_() {
        # copy file from root login to user
...
_DOUSECACHEDIR_() {
    if { [ "$USECACHEDIR" = 0 ] && [ -z "${LCR:-}" ]; } || { [ "$USECACHEDIR" = 0 ] && [ "${LCR:-}" = 3 ]; }; then
        _PPLCACHEDIR_
    fi
}
...
_SPACEINFOGSIZE_() {
    ...
    if [[ "$CPUABI" = "$CPUABIX86" ]] || [[ "$CPUABI" = "$CPUABIX8664" ]]; then
        ...
        if [[ "$USSPACE" -lt "800" ]] && [[ "$CPUABI" = "$CPUABIX8664" ]]; then
        ...
        if [[ "$USSPACE" -lt "600" ]] && [[ "$CPUABI" = "$CPUABIX86" ]]; then
...
_SPACEINFOKSIZE_() {
    ...
    elif [[ "$CPUABI" = "$CPUABIX86" ]] || [[ "$CPUABI" = "i386" ]]; then
        if [[ "$USRSPACE" -lt "600000" ]]; then
            SPACEMESSAGE="\\e[0;33mＴｅｒｍｕｘＡｒｃｈ: \\e[1;33mFREE SPACE NOTICE!  \\e[1;30mStart thinking about cleaning out some stuff please.  There is only \\e[33m$USRSPACE $units of free user space \\e[1;30mavailable on this device.  The recommended minimum to install Arch Linux in Termux PRoot for $CPUABI architecture is 600M of free user space.\\e[0m\\n"
        else
            SPACEMESSAGE=""
        fi
