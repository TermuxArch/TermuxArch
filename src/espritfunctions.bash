...
_EDITORS_() {
    ...
    for OEAEDS in "${!AEDS[@]}"; do
        if [[ -e "$PREFIX/bin/${AEDS[$OEAEDS]}" ]]; then # if editor is found
            # add editor to USEREDTR
            USEREDTR+=("${AEDS[$OEAEDS]}")
        fi
    done
    for i in "${!USEREDTR[@]}"; do
        _EDQ_
        if [[ "$EINDEX" = 1 ]]; then
            break
        fi
...
_EDQ2_() {
    while true; do
        if [[ "$OPT" = BLOOM ]] || [[ "$OPT" = MANUAL ]]; then
            ...
            read -r -n 1 -p "Answer nano or vi [n|V]? " NV
        else
...
_RMBLOOMQ_() {
    if [[ -d "$HOME"/TermuxArchBloom ]]; then
        ...
        while true; do
            printf '\n\e[1;30m'
            read -r -n 1 -p "Refresh $HOME/TermuxArchBloom? [Y|n] " RBUANSWER
