...
_I686_() {
    # IFILE is read from md5sums.txt
...
_PR00TSTRING_() {
    # construct the PRoot init statement
    ...
    if [[ -z "${KID:-}" ]]; then # command 'grep -w KID *h' shows variable KID usage
    ...
    declare -A PRST_ARR # associative array
    ...
    for PRBIND in "${!PRST_ARR[@]}"; do
        if [[ -w "$PRBIND" ]]; then # is writable
            # add proot bind
    ...
    for PRBIND in "${!PRST_ARR[@]}"; do
        if [[ -r "$PRBIND" ]]; then # is readable
            # add proot bind
            PROOTSTMNT+="-b $PRBIND:${PRST_ARR[$PRBIND]} "
        fi
    done
    ...
    PRST_ARR=(
        ['/dev/']=/dev/
        ['/dev/ashmem']="$INSTALLDIR/tmp"
        ['/dev/kvm']="$INSTALLDIR/var/binds/fbindkvm"
        ['/dev/shm']="$INSTALLDIR/tmp"
        ['/proc/loadavg']="$INSTALLDIR/var/binds/fbindprocloadavg"
        ['/proc/pcidevices']="$INSTALLDIR/var/binds/fbindprocpcidevices"
        ['/proc/shmem']="$INSTALLDIR/var/binds/fbindprocshmem"
        ['/proc/stat']="$INSTALLDIR/var/binds/fbindprocstat"
        ['/proc/uptime']="$INSTALLDIR/var/binds/fbindprocuptime"
        ['/proc/vmstat']="$INSTALLDIR/var/binds/fbindprocvmstat"
        ['/proc/version']="$INSTALLDIR/var/binds/fbindprocversion"
        ['/sys/devices']="$INSTALLDIR/var/binds/fbindsysdevices"
    )
    for PRBIND in "${!PRST_ARR[@]}"; do
        if [[ ! -r "$PRBIND" ]]; then # is not readable
            # add proot bind
            PROOTSTMNT+="-b ${PRST_ARR[$PRBIND]}:$PRBIND "
        fi
    done
