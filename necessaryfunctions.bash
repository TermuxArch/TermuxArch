...
then
sed -i "/\\#$ULANGUAGE.UTF-8 UTF-8/{s/#//g;s/@/-at-/g;}" etc/locale.gen && printf "\\e[0;32mFound an exact match for language \\e[1;32m>> %s <<\\e[0;32m to continue locale configuration.  Please run the command \\e[1;32mlocale-gen\\e[0;32m in order to set the system language." "$ULANGUAGE" # && locale-gen
else
printf "\\e[0;33mCould not find an exact match for language \\e[1;33m>> %s <<\\e[0;33m in file /etc/local.gen.  Please edit files /etc/local.conf and /etc/local.gen, and then run the command \\e[1;33mlocale-gen\\e[0;33m to set the system language.  " "$ULANGUAGE"
fi
}
