...
_ADDmakeaurpython3memoizedb_() { _PREPFILEFTN1_ "/usr/lib/python3.10/site-packages/MemoizeDB.py" python3-memoizedb python3memoizedb "generic data retrieval memoizer that uses an sqlite database to cache data" ; }

_ADDmakeaurpython3xcpf_() { _PREPFILEFTN1_ python3-xcpf python3-xcpf python3xcpf "generic data retrieval memoizer that uses an sqlite database to cache data" "{ { pacman-key -l | grep 1D1F0DC78F173680 1>/dev/null ; } || { gpg --keyserver keyserver.ubuntu.com --recv-keys 1D1F0DC78F173680 || gpg --keyserver keyserver.ubuntu.com --recv-keys 1D1F0DC78F173680 ; } ; } && { printf '\\e[0;32mCommand \\e[1;32m%s\\e[0;32m is attempting to make and install command \\e[1;32m'python3';\\e[0;32m  Please wait...\\n' \"'\${0##*/}'\" && makeaurpython3memoizedb ||: ; } &&" ; }

_ADDmakeaurpm2ml_() { _PREPFILEFTN1_ pm2ml pm2ml pm2ml "generic data retrieval memoizer that uses an sqlite database to cache data" ; }

_ADDmakeaurpython3aur_() { _PREPFILEFTN1_ python3-aur python3-aur python3aur "AUR-related modules and helper utilities (aurploader, aurquery, aurtomatic" ; }
